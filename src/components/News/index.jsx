    ...
    <div className="news">
      <div className="newsContainer">
        <div class="newsFeed">
          {filteredNews.map((props) => (
            <table className="newsTable" key={props.id}>
              <tbody>
                <tr>
                  <td className="newsImage" rowSpan="5">
                    <img src={props.content.image} alt="Imagem Noticia"></img>
                  </td>
        ...
        <div class="newsFilter">
          ...
          <button
            className={
              currentFilter === "Sem Filtro"
                ? "filterButton selected"
                : "filterButton"
            }
            onClick={() => handleFilterClick(null, "Sem Filtro")}
          >
            Sem Filtro
          </button>
          {news.map((props) =>
            filtersUsed.includes(props.content.type) ? (
              ""
            ) : filtersUsed.push(props.content.type) ? (
              <button
                className={
                  currentFilter === props.content.type
                    ? "filterButton selected"
                    : "filterButton"
                }
                onClick={() => handleFilterClick(props, props.content.type)}
              >
                {props.content.type}
              </button>
            ) : (
